...
### Рекомендация для следующей сессии (обновлено 2026-07-28, после polling consolidation)
Порядок: 1) expandable search (крупнейший оставшийся кусок), 2) wiring нормализованного endpoint к фронту (с явным fallback/сравнением против старого перед вырезанием), 3) pin/mute/archive UI поверх готового data layer, 4) read-receipts — только после архитектурного решения про per-message tracking.
