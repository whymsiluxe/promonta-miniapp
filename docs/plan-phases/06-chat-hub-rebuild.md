...
### Рекомендация для следующей сессии (обновлено 2026-07-28, после expandable search)
Порядок: 1) wiring нормализованного endpoint к фронту (с явным fallback/сравнением против старого перед вырезанием), 2) pin/mute/archive UI поверх готового data layer, 3) read-receipts — только после архитектурного решения про per-message tracking, 4) backend full-text message search, если owner подтвердит, что нужен именно он, не только текущий client-side фильтр по заголовкам/превью.
