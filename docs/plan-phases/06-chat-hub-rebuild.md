...
### FIXED (продолжение, следующая сессия)
- **Polling consolidation** — Статус: **FIXED, commit `9609941`** (см. `docs/CHANGELOG.md` за 2026-07-28 "chat polling consolidation" — тот же коммит по независимой причине несёт неточный заголовок про радио-станции из-за гонки с параллельным Telegram-bot агентом, реально закоммитившим свои радио-правки в тот же момент; содержимое проверено байт-в-байт, ничего не потеряно). Один `startChatPolling()`/`_chatPollTick()` вместо старых `_chatUnreadTimer`(15s всегда)/`_chatPollTimer`(8s только при открытом треде): AbortController отменяет незавершённый предыдущий тик, экспоненциальный backoff (4s→60s) на сетевых ошибках, пауза при `document.hidden`+немедленный tick при возврате в форграунд. "Monotonic cursor" реализован практически (backend не даёт реальной пагинации, `/api/chat/messages` по-прежнему отдаёт весь массив ≤200 сообщений) — старая эвристика `maxTs+length` заменена на сравнение сигнатуры (id+сводка реакций) снятого снимка, попутно чинит реальный пропуск: чужая реакция на видимое сообщение раньше не триггерила re-render, если ts/length не менялись. Задеплоено (`app.html`/`chat.js`/`critical-alerts.js`), verified byte-identical на `/var/www/miniapp/`.
- **Expandable search circle** — Статус: **FIXED (частично относительно буквы спеки, осознанно), commit `0d69608`.** Search теперь первый элемент той же горизонтальной ленты, что и worker-strip (не отдельный `<input>` над ней) — тап раскрывает circle на всю ширину (icon сдвигается, input фокусируется, close-кнопка справа, аватары скрываются), blur-если-пусто или × схлопывает обратно. Debounce 250ms. Query сохраняется при открытии/закрытии треда (спека: "сохранять query при возврате из найденного чата") — `_chatSearchQuery` не трогается этим флоу, при повторном `initChatView()` circle восстанавливает expanded-визуал если query непуст. Поиск расширен на таб "Общий" (раньше вообще не фильтровался) и на `last_preview` в Объекты/Дефекты/Потребности (раньше матчился только title), плюс отдельный текст "Ничего не найдено" вместо общего empty-текста при активном запросе без результатов. **Осознанное отклонение от буквы спеки**: нет отдельных состояний SEARCHING/ERROR/ABORT/AbortController — поиск остаётся client-side фильтром по уже загруженным данным (заголовки/имена/превью), backend search endpoint для полнотекстового поиска по истории сообщений не существует и не строился в этом проходе (см. "Не сделано" ниже) — имитировать сетевые состояния для операции без сетевого запроса запрещено тем же принципом спеки ("не имитировать"), что и для read receipts. Задеплоено, `data-no-swipe` заодно добавлен на весь `#chat-worker-strip` (был пропущен с момента коммита `b5becd7`, закрыт попутно раз уже трогали эту область).
- **Pin/mute/archive UI** — Статус: **FIXED, commit `ff83a1b`.** Long-press на строке треда → меню (переиспользует контейнер `.chat-bubble-menu` от реакций сообщений) с 3 toggle-пунктами, бьёт в уже существующий `POST /api/chat/threads/prefs`. Закреплённые — вверх списка внутри своей вкладки; архивные скрыты из обычного вида; ранее нефункциональная кнопка "Архив" в шапке теперь переключает режим "только архив". Prefs читаются через нормализованный `GET /api/chat/threads` как read-only источник поверх старого рендера (не замена основного пути данных — см. следующий пункт). Backend: `get_unread_by_thread` теперь пропускает заглушённые треды — mute-иконка отражает реальное подавление, не декорацию. **Побочная находка, НЕ исправлена**: `get_unread_count` (общий nav-badge) вообще не различает thread_key-треды (obj:/mangel:/task:) от group — атрибутирует их непрочитанные к `last_read` группового чата, что может давать неверный total-счётчик. Отдельный, более старый баг, не тронут в этом проходе (слишком рискованно мешать с pin/mute/archive правкой в том же файле) — см. `docs/CHANGELOG.md`.

### Не сделано (осознанно, не блокер — следующая сессия)
- **Backend full-text message search** — не строился (см. выше). Если owner хочет реальный поиск по истории сообщений (не только заголовкам/превью), это новый `GET /api/chat/search?q=...` endpoint + собственно тогда появятся настоящие SEARCHING/ERROR-состояния и станет нужен AbortController — отдельная, не факт что маленькая, задача.
...
- **`get_unread_count` thread_key-атрибуция** — найдено выше, не исправлено. Нужен отдельный, внимательный проход (общий nav-badge, вся команда его видит ежедневно).
- **Нормализованный endpoint не заменяет основной источник данных** — используется только как read-only prefs-слой (см. выше). Полная замена `/api/chat/my_threads`+`/api/workers` — отдельный, более рискованный шаг (нужно параллельно тестировать оба пути перед вырезанием старого).
- **Файловая структура из спеки** (`screens/chat-hub.js`, отдельные `components/*.js`, `core/chat-controller.js`) — всё ещё в одном `chat.js` (теперь ~1050+ строк). Не разбито намеренно: разбиение на модули без сборки (vanilla JS, `<script>`-теги) — это Architecture phase 09 забота (порядок загрузки, глобальные функции), смешивать с Chat Hub функциональными изменениями в одну сессию было бы рискованно.

### Рекомендация для следующей сессии (обновлено 2026-07-28, после pin/mute/archive)
Порядок: 1) `get_unread_count`'s thread_key-атрибуция (реальный баг, влияет на nav-badge у всех), 2) wiring нормализованного endpoint как основного источника данных для списка тредов (с явным fallback/сравнением против старого перед вырезанием), 3) read-receipts — только после архитектурного решения про per-message tracking, 4) backend full-text message search, если owner подтвердит, что нужен именно он.
