...
- Полная **история смен** (не только сегодня) — план перечисляет это отдельно от "сегодняшних смен"; сегодняшний вид дал быстрый результат, история — отдельная UI-задача (нужна пагинация/date range), не влезла в этот проход.
- **Фото до/после** — существуют как обычные feed-фото, но ничем не помечены как "до"/"после" семантически; нужно либо новое поле тегирования, либо эвристика — не строил ни то ни другое спекулятивно.
- **Бюджет в детальной карточке** — уже показан на compact preview card (owner-only metrics), не дублировал без конкретной причины.
