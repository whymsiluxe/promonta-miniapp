...
**Статус (28.07, интерактивная сессия): FIXED.** Карточка объекта уже соответствовала большей части спеки (weather island, avatar overlap max3+N, status pill, stages strip) из более ранних сессий. Реальный gap — не было вообще способа загрузить фото объекта (все карточки показывали stock fallback), и carousel для нескольких фото отсутствовал. Оба закрыты: новый `/api/objects/{object_id}/image` upload (owner-only, sniff_image validated) + delete endpoint, `object_images.json` хранит массив (до 8 фото), карточка рендерит swipeable/tappable slide-layers с dots когда 2+ фото. DTO-нормализация и формальный unit/E2E/visual regression test suite из спеки не реализованы — не давали пользовательской ценности сверх уже рабочей карточки, отложены как чисто архитектурная задача.
