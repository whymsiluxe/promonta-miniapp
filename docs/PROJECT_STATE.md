...
**(2026-07-28 update #3, supersedes the paragraph below)**: Phases 01-05 done. Phase 06 (Chat Hub full rebuild) is **in progress, not finished** — see `docs/plan-phases/06-chat-hub-rebuild.md` status section. Also this session: 2 direct live owner requests (radio station list expanded to 19 real stations + infinite-loop carousel, bottom-nav tab slide+fade transition) handled ahead of the formal phase sequence and documented in `docs/plan-phases/08-radio-player-rebuild.md`'s new status section — see `docs/CHANGELOG.md`. Current priority order for Phase 06 (per its own file): 1) consolidate the 2 chat polling timers into 1 controller (highest value, highest risk — touches live daily-used chat), 2) expandable search state machine, 3) wire the normalized `GET /api/chat/threads` endpoint to frontend, 4) pin/mute/archive UI on top of the existing backend data layer, 5) granular read receipts (blocked on an architecture decision about per-message tracking). Phases 07-10 (Object Card rebuild, remaining Radio polish, Architecture split, Tests/docs final) follow once 06 is done, per `docs/HANDOFF_PHASE05_10.md`.
