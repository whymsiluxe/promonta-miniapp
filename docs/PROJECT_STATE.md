...
**(2026-07-28 update #6, supersedes the paragraph below)**: Phases 01-05 done. Phase 06 (Chat Hub full rebuild) is **in progress, not finished** — see `docs/plan-phases/06-chat-hub-rebuild.md` status section. This session: chat polling consolidated into 1 controller, the search input merged into an expandable circle in the worker strip, and pin/mute/archive UI added over the existing backend data layer (top 3 Phase 06 priorities) — plus several live owner requests handled ahead of the formal phase sequence (radio stations expanded to 21 total + infinite carousel + mini-player/FAB overlap fix, bottom-nav tab slide+fade transition, header grid-centering fixes) — all documented in `docs/plan-phases/06-chat-hub-rebuild.md`/`08-radio-player-rebuild.md`'s status sections and `docs/CHANGELOG.md`. **Also see the "Operational risk found 2026-07-28" section above** — the concurrent-write race with the Telegram-bot agent process has now mislabeled 2 commits' content (corrected both in the docs, no data lost each time) and once made a `systemctl restart` sudo call fail transiently (succeeded on immediate retry) — treat this as a standing condition for the rest of this session, not a one-off. Current priority order for Phase 06 (per its own file): 1) fix `get_unread_count`'s thread_key mis-attribution bug (found this pass, real, affects the global nav badge for everyone), 2) wire the normalized `GET /api/chat/threads` endpoint as the primary thread-list data source (currently only used read-only for prefs), 3) granular read receipts (blocked on an architecture decision), 4) backend full-text message search (only if the owner confirms it's wanted). Phases 07-10 (Object Card rebuild, remaining Radio polish, Architecture split, Tests/docs final) follow once 06 is done, per `docs/HANDOFF_PHASE05_10.md`.
