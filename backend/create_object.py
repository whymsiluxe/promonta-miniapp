#!/usr/bin/env python3
"""Регистрация нового объекта: генерит object_id, добавляет строку в Объекты
+ строки в Этапы. Drive-папка создаётся отдельным фоновым скриптом
create_object_folder.py, чтобы не блокировать пользователя на Drive API.
