...
-- эта логика (appOutboxRecordFailure/appOutboxManualRetry в shared.js)
чистая по данным, но зависит от appOutboxPatch (IndexedDB side effect) и
...
def test_max_attempts_constant_exists_and_is_finite():
    src = _source()
    assert "const APP_OUTBOX_MAX_ATTEMPTS = 5;" in src


def test_non_transient_or_exhausted_failure_moves_to_dead_letter():
    src = _source()
    start = src.index("async function appOutboxRecordFailure(")
    end = src.index("async function appOutboxManualRetry(", start)
    body = src[start:end]
    ...
    assert "const exhausted = attempts >= APP_OUTBOX_MAX_ATTEMPTS;" in body
    ...
    assert "return appOutboxPatch(record.id, { state: 'queued', lastError:" in body
...
def test_dead_letter_is_not_picked_up_by_retry_loops_automatically():
    ...
    # themselves re-queue dead_letter records -- only appOutboxManualRetry
...
def test_manual_retry_resets_state_and_attempts_to_zero():
    src = _source()
    start = src.index("async function appOutboxManualRetry(")
    end = src.index("\n}\n", start) + len("\n}\n")
    body = src[start:end]

    assert "return appOutboxPatch(id, { state: 'queued', attempts: 0, lastError: null });" in body
